...
export const OdoUpdateModal: React.FC<OdoUpdateModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-xl w-full border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/60">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="font-bold text-base text-slate-900 dark:text-white">
              Cập Nhật Số ODO / Km Chạy Thực Tế
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-800 px-6 pt-2 bg-slate-50/50 dark:bg-slate-800/30">
          ...
          >
            Nhập nhanh cả đội ({vehicles.length} xe)
          </button>
        </div>
          ...
          <form onSubmit={handleSingleSubmit} className="p-6 space-y-4">
            ...
            <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-xl border border-slate-200 dark:border-slate-700/80 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-500 dark:text-slate-400">ODO ghi nhận trước đó:</span>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full text-xl font-bold font-mono px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    required
                  />
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-[11px] text-slate-500 mr-1">Tăng nhanh:</span>
                {[50, 100, 200, 500, 1000].map(d => (
                  <button
                    type="button"
                    key={d}
                    onClick={() => handleAddDelta(d)}
                    className="px-2 py-1 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-md text-xs font-mono font-semibold transition-colors"
                  >
                    +{d} km
                  </button>
                ))}
              ...
              {kmDifference !== 0 && (
                <div className="text-xs flex items-center justify-between pt-1 border-t border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">
                  <span>Khoảng cách di chuyển thêm:</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">
                  Ghi chú hành trình / chuyến đi
                </label>
                <input
                  type="text"
                  placeholder="Ví dụ: Đi tỉnh đón đoàn chuyên gia"
                  value={singleNote}
            ...
            <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end space-x-2">
              ...
              >
                ...
                <span>{savedSuccess ? 'Đã lưu thành công!' : 'Lưu Số ODO & Cập Nhật Cấp'}</span>
              </button>
            </div>
